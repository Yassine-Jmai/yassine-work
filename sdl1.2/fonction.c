...
enigme generer(char *nomfich)
{
    ...
    else
    {
        ...
        e.numQuestSelect = (rand() % taille) + 2;
...
void afficherEnigme(enigme e, SDL_Surface* ecran) {
    ...
    SDL_Delay(4000); //un délai de 4 secondes pour afficher l'énigme
...
int chargerEnigmes(enigme enigmes[], int maxEnigmas, char *nomfich) {
    ...
    int numEnigmasLoaded = 0;
    while (fscanf(fichier, "%s %s %s %s %d",
                   enigmes[numEnigmasLoaded].quest,
                   enigmes[numEnigmasLoaded].rep1,
                   enigmes[numEnigmasLoaded].rep2,
                   enigmes[numEnigmasLoaded].rep3,
                   &enigmes[numEnigmasLoaded].bonrep) != EOF && numEnigmasLoaded < maxEnigmas) {
        numEnigmasLoaded++;
    }

    fclose(fichier);
    return numEnigmasLoaded;
}
